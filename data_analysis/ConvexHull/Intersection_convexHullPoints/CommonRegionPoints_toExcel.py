...
if __name__ == '__main__':
    ...
    dictionary = {}
    common_lat_long = {}  # format {"region_1_and_region_2":[[x1,y1],[x2,y2],...]}
    for entry in data:
    ...
    for curr_region, curr_convex_hull_points in dict_edges_CH.items():
        ...
        for region_label, convex_hull_points in dict_edges_CH.items():
            ...
            if curr_region is not region_label and region_label not in dict_visited_regions[curr_region]:
                ...
                intersection_points = []
                # points to remember that format of convex hull lines are [[x1,x2],[y1,y2]]
                for curr_region_line in curr_convex_hull_points:
                    A = Point(curr_region_line[0][0], curr_region_line[1][0])
                    B = Point(curr_region_line[0][1], curr_region_line[1][1])
                    for region_line in convex_hull_points:
                        C = Point(region_line[0][0], region_line[1][0])
                        D = Point(region_line[0][1], region_line[1][1])
                        # import matplotlib.lines as lines
                        # fig = plt.figure()
                        # fig.add_artist(lines.Line2D([A.x,B.x,C.x,D.x], [A.y,B.y,C.y,D.y]))
                        # check any of point A/B inside convex hull of region_line and vice versa
                        curr_convex_hull = ConvexHull(np.array(dictionary[curr_region]))
                        region_convex_hull = ConvexHull(np.array(dictionary[region_label]))
                ...
                x_values = [point[0] for point in intersection_points]
                y_values = [point[1] for point in intersection_points]
                convex_hull = ConvexHull(intersection_points)
                hull_path = Path(intersection_points[convex_hull.vertices])
                print("convex hull vertices = ", convex_hull.vertices)
                # check convex hull of intersection points contains which lat, long of both region
                # check for curr region
                # common_lat_long = {} # format {"region_1_and_region_2":[[x1,y1],[x2,y2],...]}
                common_region_names = curr_region + "_" + region_label
                common_lat_long[common_region_names] = []
                for point in dictionary[curr_region]:
                    A = (point[0], point[1])
                    if hull_path.contains_point(A):
                        common_lat_long[common_region_names].append(point)
                for point in dictionary[region_label]:
                    A = (point[0], point[1])
                    if hull_path.contains_point(A):
                        common_lat_long[common_region_names].append(point)
                    # A = points
                    # if all(convex_hull.equations @ np.hstack((A, 1)) <= 0):
                    #     common_lat_long[common_region_names].append(A)
                # plot common region highlight
    ...
    # export dictionary of common lat long to excel sheet report
    print("common_lat_long = ", common_lat_long)
    # Find the maximum length among the lists
    max_length = max(len(values) for values in common_lat_long.values())

    # Fill lists with NaN to make them the same length, why need - dataframe containing list ,must have same lenght values
    for key in common_lat_long:
        common_lat_long[key] += [float('nan')] * (max_length - len(common_lat_long[key]))

    print(common_lat_long)
    # Create a DataFrame from the modified dictionary
    df_filled = pd.DataFrame(common_lat_long)
    # Export the DataFrame to an Excel file
    excel_file_path = 'common_lat_long_data.xlsx'  # Provide the desired file path
